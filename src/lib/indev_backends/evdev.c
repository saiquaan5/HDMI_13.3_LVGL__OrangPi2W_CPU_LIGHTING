...
/* Determine if mouse cursor should be hidden based on environment
 * LV_HIDE_CURSOR=[1|true|yes|on]
 */
static bool env_hide_cursor(void)
{
    const char *v = getenv("LV_HIDE_CURSOR");
    /* Mặc định ẩn nếu không đặt biến môi trường */
...
static void discovery_cb(lv_indev_t *indev, lv_evdev_type_t type, void *user_data)
{
    ...
    if((type == LV_EVDEV_TYPE_REL || type == LV_EVDEV_TYPE_ABS) && !env_hide_cursor()) {
